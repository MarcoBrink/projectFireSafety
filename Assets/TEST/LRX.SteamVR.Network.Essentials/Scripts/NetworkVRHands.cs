...
public class NetworkVRHands : NetworkBehaviour
{
	...
	void QueryController ()
	{
        ...
        if (steamDevice.GetTouchDown(SteamVR_Controller.ButtonMask.Touchpad))
        {
            if (!isTouchPressed)
        ...
        else if (steamDevice.GetTouchUp(SteamVR_Controller.ButtonMask.Touchpad))
        {
            if (isTouchPressed)
    ...
    void Update()
    {
        ...
        if (hasTouchBeenPressedThisFrame)
        {
            OnTouchPressed();
        }
        else if (hasTouchBeenReleasedThisFrame)
        {
            OnTouchReleased();
        }
    }
    ...
    void OnTouchReleased()
    {
        ...
        {
            if (hit.transform.GetComponent<Teleportable>() != null)
            {
                if (!isServer)
                {
                    RpcTeleport(hit.point);
                }
            }
        }
    }
    ...
    {
        Vector3 newPos = new Vector3(pos.x, transform.parent.position.y, pos.z);
